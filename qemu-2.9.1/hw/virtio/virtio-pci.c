...
/*virtio-pcie-pci properties*/
static Property virtio_pcie_pci_properties[] = {
    DEFINE_PROP_UINT32("class", VirtIOPCIProxy, class_code, 0),
    DEFINE_PROP_END_OF_LIST(),
};

/* virtio-pcie-pci */
static void virtio_pcie_pci_realize(VirtIOPCIProxy *vpci_dev, Error **errp)
{	
    error_report("%s is working...\n",__func__);
    //VirtIOPCIEPCI *dev = VIRTIO_PCIE_PCI(vpci_dev);
    VirtIOPCIEPCI *dev = OBJECT_CHECK(VirtIOPCIEPCI, vpci_dev, "virtio-pcie-pci");
    DeviceState *vdev = DEVICE(&dev->vdev);
...
static void virtio_pcie_pci_class_init(ObjectClass *klass, void *data)
{
    error_report("%s is working...\n",__func__);
    DeviceClass *dc = DEVICE_CLASS(klass);
    VirtioPCIClass *k = VIRTIO_PCI_CLASS(klass);
    ...
    set_bit(DEVICE_CATEGORY_STORAGE, dc->categories);
    dc->props = virtio_pcie_pci_properties;
    k->realize = virtio_pcie_pci_realize;
    pcidev_k->vendor_id = PCI_VENDOR_ID_REDHAT_QUMRANET;
    pcidev_k->device_id = PCI_DEVICE_ID_VIRTIO_PCIE;//在pci.h中定义
    pcidev_k->revision = VIRTIO_PCI_ABI_VERSION;
    pcidev_k->class_id = PCI_CLASS_OTHERS;//在pci_ids.h中定义
}

static void virtio_pcie_pci_instance_init(Object *obj)
{
    error_report("%s is working...\n",__func__);
    //VirtIOPCIEPCI *dev = VIRTIO_PCIE_PCI(obj);
    VirtIOPCIEPCI *dev = OBJECT_CHECK(VirtIOPCIEPCI, obj, "virtio-pcie-pci");

    virtio_instance_init_common(obj, &dev->vdev, sizeof(dev->vdev),
                                "virtio-pcie-device");
}

static const TypeInfo virtio_pcie_pci_info = {
    .name          = "virtio-pcie-pci",
    .parent        = TYPE_VIRTIO_PCI,
    .instance_size = sizeof(VirtIOPCIEPCI),
    .instance_init = virtio_pcie_pci_instance_init,
    .class_init    = virtio_pcie_pci_class_init,
};
...
static void virtio_pci_register_types(void)
{
    ...
    type_register_static(&virtio_9p_pci_info);
     ...
     /*注册virtio-pcie-pci设备*/
    type_register_static(&virtio_pcie_pci_info);
    //type_register_static(&virtio_pcie2_pci_info);
   // type_register_static(&virtio_pcie3_pci_info);
     /*注册virtio-pcie-net-pci设备*/
    //type_register_static(&virtio_pcie_net_pci_info);
