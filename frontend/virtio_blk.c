...
#define MAX_IO_BUFFER 717800
#define VIRTIO_ID_PCIE 21
...
/*	frontcard = (struct frontendcard *)kzalloc(sizeof(struct frontendcard),GFP_KERNEL);
	...
	dev_class = class_create(THIS_MODULE, "virtio_blk_class");
	if (IS_ERR(dev_class))
	{
		printk("Err: failed in creating class.\n");
		return -1;
	}
	device_create(dev_class, NULL, devno, NULL, "edcard");
				 ...
				 unsigned long in)
{
	...
	//	spin_lock_irq(vblk->disk->queue->queue_lock);
		if (virtqueue_add_sgs(vblk->vq, &(vbr->sg), out, in, vbr,
				      GFP_ATOMIC) < 0) {
...
//	spin_lock_irq(vblk->disk->queue->queue_lock);
	if (unlikely(virtqueue_add_sgs(vblk->vq, &(vbr->sg), out, in, vbr,
					GFP_ATOMIC) < 0)) {
	//	spin_unlock_irq(vblk->disk->queue->queue_lock);
...
//	printk("prepare to alloc req\n");
	...
	/*sg_set_buf*/
	sg_init_one(&vblk->sg[out++], &vbr->out_hdr, sizeof(vbr->out_hdr));
	sg_init_one(&vblk->sg[out++],vbr->vblk->frontendcard.io_buffer,MAX_IO_BUFFER);
	sg_init_one(&vblk->sg[num + out + in++], &vbr->status,sizeof(vbr->status));
	sgs[0] = &vblk->sg[0];
	sgs[1] = &vblk->sg[1];
	sgs[2] = &vblk->sg[2];

	if (virtqueue_add_sgs(vblk->vq, sgs, out, in, vbr,
	//if (virtqueue_add_inbuf(vblk->vq, vblk->sg, out+1, vbr,
			      GFP_ATOMIC) < 0) {
		mempool_free(vbr, vblk->pool);
		return -1;
	}
...
static int  virtblk_probe(struct virtio_device *vdev)
{
	...
	/*err = virtio_config_val(vdev, VIRTIO_PCIE_F_SEG_MAX,
				...
				&sg_elems);*/
	err = virtio_cread_feature(vdev, VIRTIO_PCIE_F_SEG_MAX,
				struct virtio_pcie_config, seg_max,
				&sg_elems);
	...
	err = virtio_cread_feature(vdev, VIRTIO_PCIE_F_BUFFLENGTH,
				struct virtio_pcie_config, bufflength,
				&bufflength);
	...
	err = virtio_cread_feature(vdev, VIRTIO_PCIE_F_FLAG,
				struct virtio_pcie_config, flag,
				&flag);
