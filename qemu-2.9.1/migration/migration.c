                 ...
                 Error **errp)
{
    ...
    if (migration_is_blocked(errp)) {//判断虚拟机状态是否适合迁移
        return;
    }

    s = migrate_init(&params);//迁移初始化
...
static void *migration_thread(void *opaque)
{
    ...
    qemu_savevm_state_header(s->to_dst_file);//保存头部

    if (migrate_postcopy_ram()) {//后拷贝代码，默认不开启，返回false
    ...
    qemu_savevm_state_begin(s->to_dst_file, &s->params);//分别调用savevm_handlers中注册的设备迁移准备接口,内存部分被注册为函数ram_save_setup
        ...
        if (!qemu_file_rate_limit(s->to_dst_file)) {
            ...
            if (pending_size && pending_size >= max_size) {
                ...
                if (migrate_postcopy_ram() &&
                    ...
                    atomic_read(&s->start_postcopy)) {//默认不使用postcopy模式
                ...
                /* Just another iteration step 第二阶段拷贝*/
                qemu_savevm_state_iterate(s->to_dst_file, entered_postcopy);

            } else {/*进行停机拷贝*/
		        error_report("迁移第二阶段已经完成，现在开始停机拷贝.\n");
    ...
    if (s->state == MIGRATION_STATUS_COMPLETED) {//迁移成功处理
        uint64_t transferred_bytes = qemu_ftell(s->to_dst_file);
